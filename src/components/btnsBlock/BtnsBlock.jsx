...
function BtnsBlock({
  ...
  return (
    <>
      <div
        className={
          { direction } === 'column' ? 'column btnsBlock' : 'row btnsBlock'
        }
        style={style}>
        <Button addClass={addBtnClass} style={btnStyle} to={btnTo}>
          {btnText}
        </Button>
        <Link className='btnsBlock__link' style={linkStyle} to={linkTo}>
          {linkText}
        </Link>
      </div>
    </>
